...
# Monkeypatch ToolManager to handle the weird Pydantic dictionary injection
# This is required because of a version mismatch or Pydantic behavior with spoon-ai-sdk
original_init = ToolManager.__init__
def patched_init(self, tools=None):
    # Handle Pydantic injection (dict) or missing args (None)
    if tools is None or (isinstance(tools, dict) and 'name' in tools):
        # print("DEBUG: Intercepted Pydantic dict injection or None. Initializing empty.")
        self.tools = []
        self.tool_map = {}
        self.indexed = False
        return
...
async def main():
    ...
    info_agent = MyInfoAgent(
        ...
        available_tools=ToolManager([SmartWeatherTool()])
    )
